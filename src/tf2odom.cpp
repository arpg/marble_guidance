...
int main(int argc, char** argv){
  ...
  ros::Subscriber cmd_sub = nh.subscribe("/X1/cmd_vel", 10, cmdCallback);
  ...
  nh.param<std::string>("X1", frame_name, "default_value");
  odom_msg.header.frame_id = "X1/map";
  new_cmd_ = false;
  ...
  while (node.ok()){
    ...
    try{
      listener.lookupTransform("/X1/map", "/X1",
